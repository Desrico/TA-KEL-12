            ...
            <table class="table table-hover mb-0" style="font-size:.85rem">
                <thead style="background:#f8fafb">
                    <tr style="color:#8898aa;font-size:.73rem;text-transform:uppercase">
                        ...
                        <th class="py-3">Terakhir Konseling</th>
                ...
                <tbody>
                    ...
                    @php($lastSession = $m->jadwalKonseling->first())
                    <tr>
                        ...
                        <td class="py-3">{{ $lastSession ? \Carbon\Carbon::parse($lastSession->tanggal)->format('d M Y') : '-' }}</td>
                        <td class="py-3">{{ $lastSession ? $lastSession->waktu . ' WIB' : '-' }}</td>
                        <td class="py-3">{{ $m->jadwalKonseling->count() }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="7" class="text-center py-5" style="color:#8898aa">Belum ada data mahasiswa</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
