            ...
            <table class="premium-table">
                ...
                <tbody>
                    ...
                    <tr>
                            ...
                            <div style="display: flex; align-items: center; gap: 16px;">
                                @if($m->thumbnail)
                                    <img src="{{ Str::startsWith($m->thumbnail, 'modules/') ? Storage::url($m->thumbnail) : $m->thumbnail }}" 
                                         alt="{{ $m->title }}" 
                                         style="width: 54px; height: 54px; border-radius: 10px; object-fit: cover; border: 1px solid #e2e8f0;">
                                @else
                                    <div style="width: 54px; height: 54px; border-radius: 10px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; border: 1px solid #e2e8f0;">🖼️</div>
                                @endif
                                <div>
                                    <div style="font-weight: 600; color: #1e293b; font-size: 1rem;">{{ $m->title }}</div>
                                    <div style="font-size: 0.8rem; color: #94a3b8; margin-top: 2px;">{{ Str::limit($m->description, 60) }}</div>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td style="font-family: 'Outfit', sans-serif; font-weight: 700; color: #059669;">{{ $m->reward_point }}</td>
                        <td>
                            <div class="actions">
